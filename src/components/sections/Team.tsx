...
const Team = () => {
  ...
  // Get current 3 members
  const getCurrentMembers = () => {
    const startIdx = currentPage * cardsPerPage;
    return teamMembers.slice(startIdx, startIdx + cardsPerPage);
  };
        ...
        <div className="flex gap-2 justify-center mt-8">
          {[...Array(totalPages)].map((_, idx) => (
            <button
              ...
              className={`w-4 h-4 rounded-full ${currentPage === idx ? 'bg-gold' : 'bg-gray-400'} transition-colors duration-300`}
